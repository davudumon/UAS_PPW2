...
<section class="p-4 bg-white rounded-lg min-h-[50vh]">
    ...
    @if (session('message'))
    <div data-alert class="relative mb-4 flex items-center justify-between rounded-md px-4 py-3 text-sm {{ $colors[$type] }}">
        <span>{{ session('message') }}</span>
        <button onclick="this.closest('[data-alert]').remove()" class="font-bold">✕</button>
    </div>
    @endif
